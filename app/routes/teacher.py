...
def analyze_file(project_id, file_index):
    ...
    # LLM ile analiz yap
    try:
        analysis_result = analyze_text_with_llm(text_chunks, categories=custom_categories)
...
""" + analysis.get('content', {}).get('ham_sonuc', ''))
    ...
    # Analiz metin dosyası olarak dışa aktar
...
def analyze_file_with_custom_headings(project_id, file_index):
    ...
    custom_prompt = """Bu PDF bir öğrenci projesidir. Aşağıdaki soruları analiz et ve her biri için KISA VE NET cevap ver:

"""

    # Başlıkları prompt'a ekle
    for i, title in enumerate(heading_titles):
        if title.strip():
            # Başlığı temizle (iki nokta üst üste kaldır)
            clean_title = title.strip().rstrip(':').rstrip('?')
            custom_prompt += f"• {clean_title}\n"

    # Basit ve net talimatlarla prompt'u tamamla
    custom_prompt += """
        İşte PDF'in içeriği:
        ...
        SADECE istenen formatta ve gerekiyorsa kısa (bir iki cümlelik açıklamayla yanıtla."""
